...
def build_deploy_base_command(config: dict, config_path: str) -> tuple[list[str], str]:
    ...
    rule_val = resolve_config_path_value(
        config_path, get_required_config_value(config, "-GENERATERULE")
    )
...
def run_nexacro_deploy_repeat(
    config: dict, config_path: str, effective_o_list: list[str], file_paths: list[str]
) -> None:
    """
