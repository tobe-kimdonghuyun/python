...
from core.config_manager import load_base_dir_from_F, load_config
from core.deploy_manager import run_nexacro_deploy_repeat
from core.file_utils import collect_files_for_FILE_from_F, compute_effective_O_values
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("config_path", help="config.json 경로")
    parser.add_argument("--run-deploy", action="store_true", help="nexacroDeployExecute를 실행합니다")

    parser.add_argument("-i", "--ignore-case", action="store_true", help="대소문자 무시")
    parser.add_argument("--contains-only", action="store_true", help="라인 전체 출력 대신 '발견 여부'만 표시")
    parser.add_argument("--max-hits", type=int, default=0, help="최대 출력 개수(0이면 제한 없음)")
    ...
    parser.add_argument("--no-line-number", action="store_true", help="줄번호 출력하지 않음")
